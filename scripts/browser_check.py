...
try:
  with sync_playwright() as p:
     ...
     page.select_option('#server-filter','gpu-server-2')
     assert page.locator('.gpu-card').count()==2
     page.select_option('#server-filter','all')
     page.fill('#gpu-search','S1-GPU3')
     ...
     page.check('[name=resources][value=S1-GPU2]')
     page.check('[name=resources][value=S1-GPU3]')
     ...
     account.check('[name=resources][value=S1-GPU0]')
