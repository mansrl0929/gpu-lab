...
가입 코드(`PORTAL_SIGNUP_CODE`)를 아는 사람만 계정을 만들 수 있습니다. 전체를 처음 상태로 되돌리려면 `bash scripts/reset_accounts.sh --yes`를 쓰며, 실행 전 사본이 `nas/backups/`에 남습니다. 코드가 외부로 알려지면 값을 바꾸고 포털만 재시작하면 됩니다(기존 계정·예약 유지). 졸업·전출자는 관리자가 `portal.sqlite3`의 `users`에서 삭제하고, 남은 예약은 관리자 권한으로 정리합니다. 비밀번호 재설정 화면은 아직 없으므로 분실 시 같은 방법으로 계정을 지우고 재가입합니다. 비밀번호는 해시로만 저장되며 관리자도 원문을 볼 수 없습니다.
